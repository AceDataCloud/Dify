...
class AceDataGrokError(RuntimeError):
    code: str
    message: str
...
class AceDataGrokClient:
    ...
    def generate_video(
        ...
        *,
        ...
        return AceDataGrokVideosResult(
    ...
    def retrieve_task(self, *, task_id: str, timeout_s: int = 60) -> dict[str, Any]:
        ...
        return self._post(path="/grok/tasks", payload=payload, timeout_s=timeout_s)
    ...
    def _post(self, *, path: str, payload: dict[str, Any], timeout_s: int) -> dict[str, Any]:
        ...
        try:
            ...
            raise AceDataGrokError(code="request_failed", message=str(e)) from e
    ...
    if isinstance(value, str):
        ...
        if text.startswith("["):
            ...
            if not isinstance(loaded, list):
                raise ValueError("`reference_image_urls` must be a JSON array or a list of strings.")
            return [str(item).strip() for item in loaded if str(item).strip()]
        return [line.strip() for line in text.splitlines() if line.strip()]

    raise ValueError(
        "`reference_image_urls` must be an array of strings or a string (one URL per line)."
    )
